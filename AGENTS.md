...
net-alpha service install                                  # one-time: write plist + sandbox, bootstrap launchd
net-alpha service start|stop|restart                       # lifecycle control
net-alpha service pause|resume                             # freeze/unfreeze background jobs (dashboard stays up)
net-alpha service status [--json]                          # health report
net-alpha service logs [-f]                                # view / tail service log
...
`net-alpha service` is an opt-in long-running launchd-supervised process that hosts
the FastAPI app and runs two background jobs:

- `price_refresh` — every 4 hours, refreshes quotes for all held + targeted tickers.
- `washsale_watch` — daily 04:00 + on every manual import. Forward-simulates each
  PositionTarget and surfaces wash-sale / §1091 risk in the Plan view as colored pills.

Architecture:
- Single FastAPI process under launchd
- Sandbox-exec profile pins writes to `~/.net_alpha/`
- Disabled-flag latch at `~/.net_alpha/run/disabled` makes Stop survive reboots
- One control module (`service/control.py`) owns all lifecycle logic
- All three management surfaces (UI, CLI, future menu-bar) call this module

Schema v19 added: `accounts.type` (taxable/IRA/Roth/401k/HSA/other),
`accounts.created_at`, plus `service_run` and `washsale_watch_result` tables.

Key files:
- `service/paths.py`, `service/control.py`, `service/scheduler.py`,
  `service/state.py`, `service/jobs/runner.py`, `service/jobs/price_refresh.py`,
  `service/jobs/washsale_watch.py`
- `engine/washsale_watch.py` — pure §1091 detection logic
...
This project is indexed by GitNexus as **net_alpha** (6177 symbols, 21061 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
