...
def test_apply_splits_is_idempotent(repo, builders):
    ...
    Idempotency is guaranteed by the canonical-formula approach: each call
    derives target_qty = trade.quantity * cumulative_ratio and skips the lot
    mutation when target_qty == current lot qty. The audit row guard
    (get_split_overrides_for_trade) ensures exactly one override row per
    (trade_id, split_id) is recorded."""
    builders.seed_import(
        repo,
        "schwab",
        "lt",
...
def test_split_survives_repeated_recompute(repo, builders):
    ...
    silently revert the lot back to raw quantities. This was the original user
    bug — SQQQ showing 100 shares instead of 10 after multi-step workflows.

    The old skip-list approach (consult lot_overrides to decide whether to
    apply) was broken because lots are REGENERATED by replace_lots_in_window
    on every recompute: the override row existed, so apply_splits skipped,
    leaving the freshly-regenerated raw lot un-adjusted.

    The canonical-formula fix always computes target_qty = trade.quantity *
    cumulative_ratio and applies it whenever the lot's current qty differs,
    making apply_splits unconditionally correct across any number of
    recomputes."""
    builders.seed_import(repo, "schwab", "lt", [
        builders.make_buy("schwab/lt", "SQQQ", date(2024, 1, 5), qty=100, cost=2000),
    ])
    ...
    # Second recompute (the bug case — simulates another CSV import triggering
    # a recompute after splits were already synced).
    recompute_all_violations(repo, load_etf_pairs())
    lots = repo.get_lots_for_ticker("SQQQ")
    assert lots[0].quantity == 10.0  # MUST stay split-adjusted, not revert to 100

    # Third recompute, for good measure.
    recompute_all_violations(repo, load_etf_pairs())
    lots = repo.get_lots_for_ticker("SQQQ")
    assert lots[0].quantity == 10.0
