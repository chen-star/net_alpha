...
def test_int_emits_interest_cash_event():
    rows = [_row(**{"Activity Date": "10/31/2024", "Trans Code": "INT", "Amount": "1.23"})]
    result = _parse(rows)
    assert len(result.cash_events) == 1
...
def test_gold_fee_emits_fee_cash_event():
    rows = [_row(**{"Activity Date": "10/01/2024", "Trans Code": "GOLD", "Amount": "-5.00"})]
    result = _parse(rows)
    assert len(result.cash_events) == 1
...
def test_ach_in_emits_transfer_in():
    rows = [
        _row(**{"Activity Date": "09/01/2024", "Trans Code": "ACH", "Amount": "1000.00", "Description": "ACH Deposit"})
    ]
    result = _parse(rows)
    assert len(result.cash_events) == 1
...
def test_ach_out_emits_transfer_out():
    rows = [_row(**{"Activity Date": "09/15/2024", "Trans Code": "ACH", "Amount": "-200.00"})]
    result = _parse(rows)
...
def test_zero_amount_cash_event_is_skipped_with_warning():
    rows = [_row(**{"Activity Date": "10/15/2024", "Instrument": "AAPL", "Trans Code": "DIV", "Amount": "0"})]
    result = _parse(rows)
    ...
    rows = [
        _row(
            **{
                ...
                "Trans Code": "DIV",
                "Amount": "12.50",
                "Description": "AAPL Cash Div Q3 2024",
            }
        )
    ]
    result = _parse(rows)
...
def test_acats_transfer_routes_correctly():
    rows = [_row(**{"Activity Date": "01/01/2024", "Trans Code": "ACATS", "Amount": "5000.00"})]
    result = _parse(rows)
    assert result.cash_events[0].kind == "transfer_in"


def test_dividend_ticker_can_be_empty():
    rows = [_row(**{"Activity Date": "10/15/2024", "Trans Code": "INT", "Amount": "0.50"})]
    result = _parse(rows)
...
def test_all_dividend_aliases_route_to_dividend(code):
    rows = [_row(**{"Activity Date": "10/15/2024", "Instrument": "AAPL", "Trans Code": code, "Amount": "10.00"})]
    result = _parse(rows)
    assert len(result.cash_events) == 1
...
def test_all_fee_aliases_route_to_fee(code):
    rows = [_row(**{"Activity Date": "10/01/2024", "Trans Code": code, "Amount": "-3.00"})]
    result = _parse(rows)
    assert len(result.cash_events) == 1
    assert result.cash_events[0].kind == "fee"
...
def test_invalid_date_cash_event_is_skipped_with_warning():
    rows = [_row(**{"Activity Date": "BADDATE", "Instrument": "AAPL", "Trans Code": "DIV", "Amount": "12.50"})]
    result = _parse(rows)
    assert result.cash_events == []
