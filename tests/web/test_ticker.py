...
def test_ticker_renders_pair_chip_and_rail_for_btc_pair(client, repo, builders):
    ...
    sto = builders.make_sto(
        "schwab/main", "AAPL", date(2024, 1, 3),
        strike=150.0, expiry=date(2024, 2, 15), call_put="P", qty=1.0, proceeds=215.0,
    )
    btc = builders.make_btc(
        "schwab/main", "AAPL", date(2024, 2, 3),
        strike=150.0, expiry=date(2024, 2, 15), call_put="P", qty=1.0, cost=5.0,
    )
...
def test_ticker_renders_still_open_for_unconsumed_lot(client, repo, builders):
    ...
    buy = builders.make_buy(
        "schwab/main", "AAPL", date(2024, 1, 10), qty=100.0, cost=14800.0
    )
