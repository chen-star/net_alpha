...
def _bootstrap_v16(engine) -> None:
    ...
    with Session(engine) as s:
        ...
        s.exec(text("INSERT INTO meta(key, value) VALUES ('schema_version', '16')"))
        s.exec(
            text(
...
def test_v17_backfills_alpha_rank():
    ...
    with Session(engine) as s:
        migrate(s)
        rows = list(s.exec(text("SELECT symbol, sort_order FROM position_targets ORDER BY symbol")).all())
...
def test_v17_migrate_is_noop_on_already_migrated_db():
    ...
    with Session(engine) as s:
        before = list(s.exec(text("SELECT symbol, sort_order FROM position_targets ORDER BY symbol")).all())
    # Run again — must not change anything.
    with Session(engine) as s:
        migrate(s)
        assert get_schema_version(s) == CURRENT_SCHEMA_VERSION
        after = list(s.exec(text("SELECT symbol, sort_order FROM position_targets ORDER BY symbol")).all())
    assert after == before
