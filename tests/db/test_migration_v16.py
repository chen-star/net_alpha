...
def _bootstrap_v15(engine) -> None:
    ...
    with Session(engine) as s:
        ...
        s.exec(text("INSERT INTO meta(key, value) VALUES ('schema_version', '15')"))
        s.exec(
            text(
...
def test_v16_creates_position_target_tag_table():
    ...
    with Session(engine) as s:
        ...
        idxs = list(
            s.exec(text("SELECT name FROM sqlite_master WHERE type='index' AND tbl_name='position_target_tag'")).all()
        )
...
def test_v16_cascade_on_target_delete():
    ...
    with Session(engine) as s:
        ...
        s.exec(text("INSERT INTO position_target_tag(target_symbol, tag) VALUES ('HIMS', 'core')"))
        s.commit()
        s.exec(text("DELETE FROM position_targets WHERE symbol='HIMS'"))
        s.commit()
        remaining = s.exec(text("SELECT COUNT(*) FROM position_target_tag WHERE target_symbol='HIMS'")).first()
