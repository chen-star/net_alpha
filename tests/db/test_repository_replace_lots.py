...
def test_replace_lots_in_window_persists_lots_from_detection(repo):
    ...
    rec = ImportRecord(
        account_id=a.id, csv_filename="x.csv", csv_sha256="h",
        imported_at=datetime(2026, 4, 25), trade_count=0,
    )
    trades = [
        Trade(account=a.display(), date=date(2024, 6, 5), ticker="TSLA",
              action="Buy", quantity=10, cost_basis=1700.0),
        Trade(account=a.display(), date=date(2024, 6, 15), ticker="AAPL",
              action="Buy", quantity=5, cost_basis=750.0),
    ]
...
def test_replace_lots_in_window_does_not_touch_lots_outside_window(repo):
    ...
    repo.add_import(a, rec, [
        Trade(account=a.display(), date=date(2024, 1, 1), ticker="OUT_OF_WIN",
              action="Buy", quantity=5, cost_basis=100.0),
        Trade(account=a.display(), date=date(2024, 6, 5), ticker="IN_WIN",
              action="Buy", quantity=10, cost_basis=200.0),
    ])
