...
def test_render_starts_with_version():
    text = sandbox.render(net_alpha_home="/u/.net_alpha", port=18765)
...
def test_render_allows_loopback_bind_on_pinned_port():
    ...
    assert '"localhost:18765"' in text
...
def test_render_allows_loopback_inbound_on_pinned_port():
    ...
    assert text.count('"localhost:18765"') >= 2  # bind + inbound both pinned to port
