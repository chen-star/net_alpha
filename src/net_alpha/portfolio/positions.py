...
def open_lots_view(
    *,
    ...
    for lot, rem_qty, rem_basis in consumed:
        ...
        # Audit (2026-05-17): every Lot field is accounted for below.
        #   - Passed through unchanged: id, trade_id, account, date, ticker, option_details
        #   - Prorated by `ratio`: quantity, cost_basis
        #   - Set from `consume_lots_fifo`'s rem_basis (post-wash-sale per-share
        #     basis already scaled to the remaining qty): adjusted_basis
        #   - Passed through unchanged (historically dropped, fixed in Task 4):
        #     tacked_acquired_date
        # If new Lot fields are added in the future, verify each belongs here.
        out.append(
            Lot(
                id=lot.id,
